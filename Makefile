...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_mapcolumns_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_mapcolumns-*.tar.gz
	echo Create archive \"pp_cmd_mapcolumns-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_mapcolumns-$(VERSION)-$(BRANCH).tar.gz mapcolumns

clean_pack:
	rm -f pp_cmd_mapcolumns-*.tar.gz


pp_cmd_mapcolumns.tar.gz: build
	cd make_build; tar czf ../pp_cmd_mapcolumns.tar.gz mapcolumns && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./mapcolumns make_build
	cp *.md make_build/mapcolumns/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/mapcolumns
...
test:
	docker build --progress=plain -t pp_cmd_mapcolumns:$(VERSION) . && docker run -v $$(pwd):/app -it pp_cmd_mapcolumns:$(VERSION) python -m unittest tests/test_command.py
